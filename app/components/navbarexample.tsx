  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 z-[90] bg-background/95 backdrop-blur-xl transition-all duration-500 flex items-center justify-center ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <div className="flex flex-col items-center space-y-2 md:space-y-4 text-center py-12">
          {subpages.map((page, i) => (
            <Link 
              key={i} 
              href={page.slug}
              onClick={() => setIsOpen(false)}
              className="text-3xl md:text-5xl font-black tracking-tighter uppercase hover:text-stripe transition-all duration-300"
              style={{ 
